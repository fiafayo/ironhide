...
abstract class BaseJadwalUjian extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array RuangBlock[] Collection to store aggregation of RuangBlock objects.
	 */
	protected $collRuangBlocks;

	/**
	 * @var        Criteria The criteria used to select the current contents of collRuangBlocks.
	 */
	private $lastRuangBlockCriteria = null;
	...
	/**
	 * @var        array KaryawanBlock[] Collection to store aggregation of KaryawanBlock objects.
	 */
	protected $collKaryawanBlocks;

	/**
	 * @var        Criteria The criteria used to select the current contents of collKaryawanBlocks.
	 */
	private $lastKaryawanBlockCriteria = null;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 9; // 9 = JadwalUjianPeer::NUM_COLUMNS - JadwalUjianPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->aMataKuliah = null;
			$this->collRuangBlocks = null;
			$this->lastRuangBlockCriteria = null;

			$this->collDosenBlocks = null;
			$this->lastDosenBlockCriteria = null;

			$this->collKaryawanBlocks = null;
			$this->lastKaryawanBlockCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collRuangBlocks !== null) {
				foreach ($this->collRuangBlocks as $referrerFK) {
			...
			if ($this->collKaryawanBlocks !== null) {
				foreach ($this->collKaryawanBlocks as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getRuangBlocks() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addRuangBlock($relObj->copy($deepCopy));
				}
			}
			...
			foreach ($this->getKaryawanBlocks() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addKaryawanBlock($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collRuangBlocks collection (array).
	 *
	 ...
	 * @see        addRuangBlocks()
	 */
	public function clearRuangBlocks()
	{
		$this->collRuangBlocks = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collRuangBlocks collection (array).
	 *
	 * By default this just sets the collRuangBlocks collection to an empty array (like clearcollRuangBlocks());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initRuangBlocks()
	{
		$this->collRuangBlocks = array();
	}

	/**
	 * Gets an array of RuangBlock objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this JadwalUjian has previously been saved, it will retrieve
	 * related RuangBlocks from storage. If this JadwalUjian is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array RuangBlock[]
	 * @throws     PropelException
	 */
	public function getRuangBlocks($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(JadwalUjianPeer::DATABASE_NAME);
		}
		...
		if ($this->collRuangBlocks === null) {
			if ($this->isNew()) {
			   $this->collRuangBlocks = array();
				...
				$criteria->add(RuangBlockPeer::KODE_UJIAN, $this->kode_ujian);

				RuangBlockPeer::addSelectColumns($criteria);
				$this->collRuangBlocks = RuangBlockPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				RuangBlockPeer::addSelectColumns($criteria);
				if (!isset($this->lastRuangBlockCriteria) || !$this->lastRuangBlockCriteria->equals($criteria)) {
					$this->collRuangBlocks = RuangBlockPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastRuangBlockCriteria = $criteria;
		return $this->collRuangBlocks;
	}

	/**
	 * Returns the number of related RuangBlock objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related RuangBlock objects.
	 * @throws     PropelException
	 */
	public function countRuangBlocks(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(JadwalUjianPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collRuangBlocks === null) {
			if ($this->isNew()) {
				...
				$count = RuangBlockPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastRuangBlockCriteria) || !$this->lastRuangBlockCriteria->equals($criteria)) {
					...
					$count = count($this->collRuangBlocks);
				}
			} else {
				$count = count($this->collRuangBlocks);
			}
	...
	/**
	 * Method called to associate a RuangBlock object to this object
	 * through the RuangBlock foreign key attribute.
	 *
	 * @param      RuangBlock $l RuangBlock
	 * @return     void
	 * @throws     PropelException
	 */
	public function addRuangBlock(RuangBlock $l)
	{
		if ($this->collRuangBlocks === null) {
			$this->initRuangBlocks();
		}
		if (!in_array($l, $this->collRuangBlocks, true)) { // only add it if the **same** object is not already associated
			array_push($this->collRuangBlocks, $l);
			$l->setJadwalUjian($this);
		}
	}
	 ...
	 * Otherwise if this JadwalUjian is new, it will return
	 * an empty collection; or if this JadwalUjian has previously
	 * been saved, it will retrieve related RuangBlocks from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in JadwalUjian.
	 */
	public function getRuangBlocksJoinRuang($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(JadwalUjianPeer::DATABASE_NAME);
		}
		...
		if ($this->collRuangBlocks === null) {
			if ($this->isNew()) {
				...
				$this->collRuangBlocks = RuangBlockPeer::doSelectJoinRuang($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this JadwalUjian has previously been saved, it will retrieve
	 * related DosenBlocks from storage. If this JadwalUjian is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collDosenBlocks === null) {
			if ($this->isNew()) {
			   $this->collDosenBlocks = array();
				...
				$criteria->add(DosenBlockPeer::KODE_UJIAN, $this->kode_ujian);
	 ...
	 * actually need in JadwalUjian.
	 */
	public function getDosenBlocksJoinDosen($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(JadwalUjianPeer::DATABASE_NAME);
		}
		...
		if ($this->collDosenBlocks === null) {
			if ($this->isNew()) {
				...
				$this->collDosenBlocks = DosenBlockPeer::doSelectJoinDosen($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collKaryawanBlocks collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addKaryawanBlocks()
	 */
	public function clearKaryawanBlocks()
	{
		$this->collKaryawanBlocks = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collKaryawanBlocks collection (array).
	 *
	 * By default this just sets the collKaryawanBlocks collection to an empty array (like clearcollKaryawanBlocks());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initKaryawanBlocks()
	{
		$this->collKaryawanBlocks = array();
	}

	/**
	 * Gets an array of KaryawanBlock objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this JadwalUjian has previously been saved, it will retrieve
	 * related KaryawanBlocks from storage. If this JadwalUjian is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array KaryawanBlock[]
	 * @throws     PropelException
	 */
	public function getKaryawanBlocks($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(JadwalUjianPeer::DATABASE_NAME);
		}
		...
		if ($this->collKaryawanBlocks === null) {
			if ($this->isNew()) {
			   $this->collKaryawanBlocks = array();
				...
				$criteria->add(KaryawanBlockPeer::KODE_UJIAN, $this->kode_ujian);

				KaryawanBlockPeer::addSelectColumns($criteria);
				$this->collKaryawanBlocks = KaryawanBlockPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				KaryawanBlockPeer::addSelectColumns($criteria);
				if (!isset($this->lastKaryawanBlockCriteria) || !$this->lastKaryawanBlockCriteria->equals($criteria)) {
					$this->collKaryawanBlocks = KaryawanBlockPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastKaryawanBlockCriteria = $criteria;
		return $this->collKaryawanBlocks;
	}

	/**
	 * Returns the number of related KaryawanBlock objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related KaryawanBlock objects.
	 * @throws     PropelException
	 */
	public function countKaryawanBlocks(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(JadwalUjianPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collKaryawanBlocks === null) {
			if ($this->isNew()) {
				...
				$count = KaryawanBlockPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastKaryawanBlockCriteria) || !$this->lastKaryawanBlockCriteria->equals($criteria)) {
					...
					$count = count($this->collKaryawanBlocks);
				}
			} else {
				$count = count($this->collKaryawanBlocks);
			}
	...
	/**
	 * Method called to associate a KaryawanBlock object to this object
	 * through the KaryawanBlock foreign key attribute.
	 *
	 * @param      KaryawanBlock $l KaryawanBlock
	 * @return     void
	 * @throws     PropelException
	 */
	public function addKaryawanBlock(KaryawanBlock $l)
	{
		if ($this->collKaryawanBlocks === null) {
			$this->initKaryawanBlocks();
		}
		if (!in_array($l, $this->collKaryawanBlocks, true)) { // only add it if the **same** object is not already associated
			array_push($this->collKaryawanBlocks, $l);
			$l->setJadwalUjian($this);
		}
	}
	 ...
	 * Otherwise if this JadwalUjian is new, it will return
	 * an empty collection; or if this JadwalUjian has previously
	 * been saved, it will retrieve related KaryawanBlocks from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in JadwalUjian.
	 */
	public function getKaryawanBlocksJoinKaryawan($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(JadwalUjianPeer::DATABASE_NAME);
		}
		...
		if ($this->collKaryawanBlocks === null) {
			if ($this->isNew()) {
				...
				$this->collKaryawanBlocks = KaryawanBlockPeer::doSelectJoinKaryawan($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	{
		if ($deep) {
			if ($this->collRuangBlocks) {
				foreach ((array) $this->collRuangBlocks as $o) {
			...
			if ($this->collKaryawanBlocks) {
				foreach ((array) $this->collKaryawanBlocks as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)

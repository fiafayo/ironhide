...
while ($input) {
  ...
  if ($nrp && $mk && $nilai) {
    ...
    try {
      echo "NEW $nrp $mk ... ";
      $sql="INSERT INTO tk_transkrip_asli (nrp,kode_mk,semester,tahun,nilai) VALUES ('$nrp','$mk','$semester','$tahun','$nilai')";
    ...
    try {
      echo "EDIT $nrp $mk ... ";
      $sql="UPDATE tk_transkrip_asli SET nilai='$nilai' WHERE nrp='$nrp' and  kode_mk='$mk' and semester='$semester' and tahun='$tahun'";
      $stmt=$con->prepare($sql);
      $stmt->execute();
      echo "$no \n";
    } catch (Exception $e) {

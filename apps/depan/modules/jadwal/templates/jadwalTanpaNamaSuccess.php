...
if (!$xls) {
  ...
  print '<input type="hidden" name="commit" value="Bikin Jadwal Tanpa Nama Dosen" />';
  print '<input type="submit" name="Commit" value="Klik disini untuk membuat file XLS" />';
  print '</form></div>';
}
?>
...
<TR>
  ...
  <TH style="text-align:center" colspan="9">Dosen</TH>
...
<TR class="sf_admin_row_<?php print $genap;?>">
  ...
  if ($kelasMk) {
    ...
    foreach ($dosenKelass as $dosenKelas) {
        $n++;
        echo '<td>'.$dosenKelas->getKodeDosen().'</td>';
    }
    for ($d=$n+1; $d<=9; $d++) {
        echo '<td>&nbsp;</td>';
    }
